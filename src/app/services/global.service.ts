...
export class GlobalService {
  // pagesArray: number[] = [];
  public urlPrev="";
  private categoriesUrl = 'http://localhost:8090/api/collections/syCategories/records';
  private productsUrl = 'https://db.buckapi.com:8090/api/collections/frutmeProducts/records';
/*   private doctorsUrl = 'https://db.buckapi.com:8090/api/collections/doctors/records';
 */  private specialtiesUrl = 'https://db.buckapi.com:8090/api/collections/syCategories/records';
  private segurosUrl = 'https://db.buckapi.com:8090/api/collections/syProducts/records';
...
this.getSpecialties().subscribe(
  response => {
    this.syCategories = response;
  }
);
this.getProducts().subscribe(
  response => {
    this.syProducts = response;
  }
);
   ...
   getSpecialties(): Observable<any[]> {
    ...
    return this.getAllPages(this.urlPrev).pipe(
      tap(syCategories => {
        ...
        syCategories.forEach(product => categories.add(product.cat)); // Asumiendo que 'cat' es la propiedad que contiene la categoría de cada producto
  ...
  getProducts(): Observable<any[]> {
    this.urlPrev = this.segurosUrl;
    return this.getAllPages(this.urlPrev).pipe(
      tap(syProducts => {
        // Extraer todas las productos encontradas
        const products: Set<string> = new Set(); // Aquí establecemos explícitamente que el conjunto contendrá cadenas de texto
        
        syProducts.forEach(product => products.add(product.cat)); // Asumiendo que 'cat' es la propiedad que contiene la categoría de cada producto
        
        // Ordenar las productos alfabéticamente
        this.products = Array.from(products).sort((a, b) => a.localeCompare(b));
      })
    );
  }
